    ...
    <div className="space-y-4">
        ...
        {features.length > 0 ? (
          <div className="flex flex-wrap gap-2 p-3 bg-slate-50 border border-slate-200 rounded-2xl min-h-[50px]">
            {features.map((feat, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#D4AF37]/50 text-[#3D1347] rounded-xl text-xs font-semibold shadow-xs"
              >
      ...
      <form onSubmit={handleAddCustom} className="flex gap-2">
        <input
          ...
          className="flex-1 px-3.5 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-[#3D1347]"
        />
        <button
          type="submit"
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer border ${
                  isSelected
                    ? 'bg-[#3D1347] text-[#D4AF37] border-[#3D1347] shadow-xs'
