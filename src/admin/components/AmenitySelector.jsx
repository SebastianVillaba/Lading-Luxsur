    ...
    <div className="space-y-4">
        ...
        {features.length > 0 ? (
          <div className="flex flex-wrap gap-2 p-3 bg-slate-50 border border-slate-200 rounded-2xl min-h-[50px]">
            {features.map((feat, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-purple-200 text-[#5e265e] rounded-xl text-xs font-semibold shadow-xs"
              >
      ...
      <form onSubmit={handleAddCustom} className="flex gap-2">
        <input
          ...
          className="flex-1 px-3.5 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-[#5e265e]"
        />
        <button
          type="submit"
      ...
      <div>
        <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-2 flex items-center gap-1">
          <Sparkles className="w-3 h-3 text-[#5e265e]" />
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer border ${
                  isSelected
                    ? 'bg-[#5e265e] text-white border-[#5e265e] shadow-xs'
